...
// 	sp_mat A = vectorizers::count_vectorize(x_train,grams);
    sp_mat A = vectorizers::tfidf_vectorize(x_train,grams);
	...
	// n-grams works but our gradient descent in logreg is not fast enough for very large matrices.
	// we will probably implement linear search in the near future.
	double C = 10.,tol = 1e-5,alpha = 0.0002;
